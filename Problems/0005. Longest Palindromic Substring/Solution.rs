impl Solution {
    pub fn longest_palindrome(s: String) -> String {
        ...
        for i in 0..len {
            Self::expand(s_chars, i as i32, i as i32, &mut max_len, &mut start);
            Self::expand(s_chars, i as i32, i as i32 + 1, &mut max_len, &mut start);
        }

        return s_chars[start..start + max_len].iter().map(|&c| c as char).collect();
    }

    fn expand(s: &[u8], mut l: i32, mut r: i32, max_len: &mut usize, start: &mut usize) {
